...
<body>
                                    ...
                                    <tr class="text-gray-700 dark:text-gray-100">
                                        ...
                                        <td
                                            ...
                                            <button data-modal-target="crud-modal" data-modal-toggle="crud-modal"
                                                ...
                                                type="button" onclick="openModal({{ $category->id }})">
                                                Modifier
                                            </button>
                                            <!-- Include your input fields for updating here -->
                                        ...
                                        <td
                                            ...
                                            <div class="flex items-center">
                                                <form action="{{ route('destroycategory', ['id' => $category->id]) }}"
                                                    method="post"
                            ...
                            <div
                                ...
                                <div class="flex-grow flex justify-between items-center">
                                    ...
                                    <form method="POST" action="{{ route('users.update', $usr) }}">
                                        ...
                                        <button type="submit"
                                            class="bg-{{ $usr->status ? 'green' : 'red' }}-500 text-white text-sm px-3 py-1 rounded">
    ...
    <div class="mt-8 w-1/2 ml-64">
        <table class="min-w-full bg-white border border-gray-300 table-width">
            <thead>
                <tr>
                    <th class="py-2 px-4 border-b border-gray-300 text-xs uppercase font-semibold text-left">Event</th>

                    <th class="py-2 px-4 border-b border-gray-300 text-xs uppercase font-semibold text-left">Action
                    </th>
                </tr>
            </thead>
            <tbody>
                @foreach ($event as $events)
                    <tr class="text-gray-700 dark:text-gray-100">
                        ...
                        <td class="py-2 px-4 border-b border-gray-200">
                            <form action="{{ route('validateev', $events->id) }}" method="POST">
                                @csrf
                                @method('put')
                                <input type="hidden" name="statut" value="accepted">
                                <button type="submit"
                                    class="text-white bg-green-600 px-3 rounded-full mr-4">Publier</button>

                            </form>

                            <form action="{{ route('rejectev', $events->id) }}" method="POST">
                                @csrf
                                @method('put')
                                <input type="hidden" name="statut" value="rejected">
                                <button type="submit"
                                    class="bg-red-400 text-white text-xs py-1 px-2 rounded">Rejeter</button>
                            </form>
                        </td>
                    </tr>
            </tbody>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <form action="{{ route('updatecategory', ['id' => $category->id]) }}" method="post">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <!-- Modal header -->
                    <input type="hidden" name="categorie_id" value="{{ $category->id }}" id="category_id">
    ...
    <div id="popupAjoutCategorie"
        class="fixed bg-gray-700 bg-opacity-50 hidden top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ">
