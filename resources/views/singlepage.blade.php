...
<div class="relative">
    ...
    <section class="bg-yellow-50 overflow-hidden">
        <div class="flex flex-col lg:flex-row lg:items-stretch lg:min-h-[800px]">
            <div>
                <form action="{{ route('reservation.store', ['id' => $event->id, 'clientId' => auth()->id()]) }} " method="post">
                    @csrf
                    <button id="bookNowBtn" type="submit" class="inline-flex items-center justify-center w-full px-4 py-4 mt-4 font-semibold text-white transition-all duration-200 bg-orange-500 border border-transparent rounded-full sm:w-auto sm:ml-4 sm:mt-0 hover:bg-orange-600 focus:bg-orange-600">
                        Book Now
                    </button>
                </form>
            </div>
