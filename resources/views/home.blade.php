    ...
    <section class="bg-[#FCF8F1] bg-opacity-30 py-10 sm:py-16 lg:py-24">
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="grid items-center grid-cols-1 gap-12 lg:grid-cols-2">
                <div>
                    <h1 class="mt-4 text-4xl font-bold text-black lg:mt-8 sm:text-6xl xl:text-8xl"> Laissez la magie des
                        événements prendre vie</h1>
                    <p class="mt-4 text-base text-black lg:mt-8 sm:text-xl">Créez des souvenirs uniques avec notre
                        plateforme de gestion et réservation </p>
...
<div class="flex justify-center gap-5">
    @foreach ($categories as $category)
        <a href="{{ route('home', ['id' => $category->id]) }}"
            class="px-4 py-2 text-white bg-yellow-300 rounded">{{ $category->name }}</a>
    @endforeach
</div>
        ...
        <div class="flex items-center">
            ...
            <form action="{{ route('home.search') }}" method="GET">
                <input type="text" name="search_title"
                    class="px-2 py-1 border border-gray-300 rounded focus:outline-none focus:border-blue-600"
                ...
                <div class="p-5">
                    <div class="relative">
                        <a href="#" title="" class="block aspect-w-4 aspect-h-3">
                            <img class="object-cover w-full h-full" src="{{ url('storage/images/' . $event->image) }}"
                                alt="" />
                        </a>
                    ...
                    <span
                        class="block mt-6 text-sm font-semibold tracking-widest text-gray-500 uppercase">{{ $event->date }}</span>
                    <p class="mt-5 text-2xl font-semibold">
                        <a href="#" title="">{{ $event->name }}</a>
                    </p>
                    <p class="mt-4 text-base text-gray-600">{{ $event->description }}</p>
                    <a href="{{ route('event.show', ['id' => $event->id]) }}" title=""
